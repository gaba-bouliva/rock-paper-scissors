...
function getComputerChoice () {
  ...
  const randomChoice = Math.floor((Math.random() * 3)) + 1 // returns random number between 1 and 3
  if (randomChoice === 1) {
    return 'rock'
  }
  if (randomChoice === 2) {
...
function playRound (userChoice) {
  ...
  if (userChoice === computerChoice) {
    return { computerChoice, message: 'Draw Game!' }
  }

  if (userChoice === 'rock' && computerChoice === 'paper') {
    return { computerChoice, message: `You Lose! ${computerChoice} beats  ${userChoice}.` }
  }

  if (userChoice === 'rock' && computerChoice === 'scissors') {
    return { computerChoice, message: `You Win! ${userChoice} beats ${computerChoice}.` }
  }
